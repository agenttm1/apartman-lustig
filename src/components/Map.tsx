...
export default function Map({ dict }: MapProps) {
  ...
  return (
    <div className="w-full h-[500px] relative z-0 rounded-3xl overflow-hidden shadow-xl border border-gray-100 bg-gray-50 my-12 max-w-7xl mx-auto">
      <iframe
        src={embedUrl}
        width="100%"
        height="100%"
      ...
      <div className="absolute bottom-0 left-0 w-full h-32 bg-gradient-to-t from-black/60 to-transparent z-10 pointer-events-none"></div>
      ...
      <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 z-20 w-[90%] max-w-sm">
        ...
        >
          ...
          {/* Dinamički tekst gumba za navigaciju */}
          <span className="font-bold tracking-wide uppercase text-sm">{dict.button}</span>
        </a>
      </div>
    </div>
  );
