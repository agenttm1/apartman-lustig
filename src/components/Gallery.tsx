...
export default function Gallery({ dict }: GalleryProps) {
  ...
  const images = [
    { id: 1, src: "/gallery/slika1.jpg", alt: dict.img1 },
    { id: 2, src: "/gallery/slika2.jpg", alt: dict.img2 },
    { id: 3, src: "/gallery/slika3.jpg", alt: dict.img3 },
    { id: 4, src: "/gallery/slika4.jpg", alt: dict.img4 },
  ];
    ...
    <section className="relative w-full h-screen bg-[#0a1f18] flex flex-col items-center justify-center overflow-hidden">
      <div className="absolute top-10 left-6 md:left-12 z-20">
        <h2 className="text-4xl md:text-5xl font-bold text-white tracking-tight font-serif drop-shadow-md">
          {dict.title} <span className="text-[#d4af37]">{dict.titleAccent}</span>
        </h2>
      </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <Image
              ...
              priority
              className="transition-transform duration-[2000ms] ease-out hover:scale-105"
            />
